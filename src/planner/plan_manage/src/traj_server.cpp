...
void publish_cmd(Vector3d p, Vector3d v, Vector3d a, Vector3d j, double y, double yd)
{
   if(land_ready==true)
   {
    ...
    land_ready_pub.publish(msg);
   }
    Vector3d target_position(0, 0, 0.5);  
    Vector3d zero_velocity(0, 0, 0);    

    double position_tolerance = 0.05;   
    double velocity_tolerance = 0.01;   

    Vector3d position_error = p - target_position;
    Vector3d velocity_error = v - zero_velocity;

    bool reached_target = (position_error.norm() < position_tolerance) && (velocity_error.norm() < velocity_tolerance);
...
int main(int argc, char **argv)
{
  ...
  land_ready_pub=nh.advertise<quadrotor_msgs::TakeoffLand>("/px4ctrl/takeoff_land",1);
